...
def get_column_picklist(table_name: str, column_name: str, db_id: str) -> list:
    fetch_sql = "SELECT DISTINCT {} FROM {}.{}".format(column_name, db_id, table_name)
    conn = None
    try:
        conn = pymssql.connect("localhost", "SA", "Passw0rd!", as_dict=False, autocommit=False)
        #conn.text_factory = bytes
        c = conn.cursor()
        c.execute("USE spider")
        c.execute(fetch_sql)
        picklist = set()
        for x in c.fetchall():
...
def get_database_matches(
    ...
    picklist = get_column_picklist(
        table_name=table_name, column_name=column_name, db_id=db_id
    )
